...
icu_s_corrs = read.csv('~/Desktop/miRNA/rnaseq/outputs/icu_corrs_spearman_sig_fdr05_oasis_TableS4B.csv')
...
# IPA targets
main_df = load_icu_ipa_data(ipa_results_dir, 'icu_notgender_mirna_targets.xls')

# Annotate miRNA-RNA pairs 
icu_s_corrs_ann = annotate_mirna_table(icu_s_corrs)

write.csv(icu_s_corrs_ann, 
          '~/Desktop/miRNA/rnaseq/outputs/corrs_spearman_sig_fdr05_icu_notgender_oasis_annotated.csv')
...
highly_predicted = icu_s_corrs_neg_ann %>%
  ...
  dplyr::select(mirna, ref_gene_name) %>%
  distinct()
...
df = icu_s_corrs_neg_ann %>%
  filter(confidence %in% c('Experimentally Observed',
                           'High (predicted)')) %>%
  dplyr::select(mirna, rna, ref_gene_name, corr, FDR_P, Significance, confidence) %>%
...
#-------------------------------------------------#
write.csv(df, '~/Desktop/covid_paper_outputs_oasis/Supplementary_Table_5.csv', row.names=FALSE)
...
p = ggarrange(p6, p7, p24, p9, p10, p28, nrow=2, ncol=3,
              label.x = 0, label.y = 1, 
...
#---------------------------------------------#
png('~/Desktop/covid_paper_outputs_oasis/Supplementary_Figure_5_part2.png', width=900, height=500)
plot(p)
dev.off()
